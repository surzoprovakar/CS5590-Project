...
# mode = 0 --> without trust, mode = 1 --> with trust
mode = 1
RANDOM_SEED = 42
SIM_TIME = 300
...
def message_generator(name, env, out_pipe):
    ...
    while True:
        ...
        msg = (env.now, '%s sends %s at time %d as %s' %
               (name, ciphertext, env.now, opt))
        out_pipe.put(msg)
...
def message_consumer(name, env, in_pipe, value, trust_dict, c, conn):
    ...
    while True:
        ...
        if mode == 1:
            ...
            print('Type %s| Decision %s| Updated Trust %f' %
                  (opt_type, decision, trust_dict[requester]))
            print(trust_dict)
...
cA, connA, cB, connB, cC, connC, cD, connD = db_generatoor(mode)
...
env.process(message_consumer('Replica A', env, bc_pipe.get_output_connA(), 0, {
            'A': 50, 'B': 50, 'C': 50, 'D': 50}, cA, connA))
env.process(message_consumer('Replica B', env, bc_pipe.get_output_connA(), 0, {
            'A': 50, 'B': 50, 'C': 50, 'D': 50}, cB, connB))
env.process(message_consumer('Replica C', env, bc_pipe.get_output_connA(), 0, {
            'A': 50, 'B': 50, 'C': 50, 'D': 50}, cC, connC))
env.process(message_consumer('Replica D', env, bc_pipe.get_output_connA(), 0, {
            'A': 50, 'B': 50, 'C': 50, 'D': 50}, cD, connD))
